...
async def image_optimal_advance(file_path_dir):
    try:
        image = cv2.imread(file_path_dir)

        ratio = 2000.0 / image.shape[1]
        dim = (2000, int(image.shape[0] * ratio))

        scale_image = cv2.resize(image, dim, interpolation=cv2.INTER_AREA)
        cv2.imwrite(file_path_dir, scale_image)
    except Exception as ex:
        print("Loi me no roi (image_optimal_advance): ", ex)
